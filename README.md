# Wildcat Website Monitor 🐱

This is an automated tool that checks the Wildcat website every 5 minutes. If it finds that "work is available" (meaning the "all tasks picked up" message is gone), it sends you a message on Telegram instantly.
...
3. Give it a name (e.g., "Wildcat Alert").
4. Give it a unique username (e.g., `WildcatAlert123_bot`).
...
### Secret #5
**Name**: `TARGET_URL`
**Secret**: `https://wildcat.cgtrader.com/modeling-requests`

---

### Secret #6
**Name**: `LOGIN_URL`
**Secret**: `https://wildcat.cgtrader.com/users/login`
...
A: This usually means there are no tasks (the specific "not found" phrase is present). Check the "Actions" tab in GitHub to see if the "Wildcat Website Checker" workflow is running green.
...
**Q: I want to stop it.**
A: Go to the "Actions" tab, click "Wildcat Website Checker" on the left, check the three dots `...` and select "Disable workflow".
